...
public class MSSQLServerJsonTests
{
    ...
    public void OneTimeSetup()
    {
        ...
        using (var connection = GetDbConnection())
            connection.Execute(@"create table apiresponsehistories(id int identity(1,1) primary key, parameters varchar(max) null, response varchar(max) null)");

        Console.WriteLine("[SQLServer-Json] One Time Setup Completed");
    }

    [SetUp]
    public void Setup()
    {
        Console.WriteLine("[SQLServer-Json] Test Setup Completed");
    }

    [TearDown]
    public void TearDown()
    {
        Console.WriteLine("[SQLServer-Json] Test Completed");
    }
    ...
    public void Insert(HttpStatusCode statuscode, string message, decimal amount, int qty, string requestId, string requstPath)
    {
        ...
        Console.WriteLine("[SQLServer-Json] New entry created. [Entry={0}]", JsonConvert.SerializeObject(apiResponseHistory));

        var row = connection.Get<APIResponseHistory>(id);
        Console.WriteLine("[SQLServer-Json] Read created entry. [Entry={0}]", JsonConvert.SerializeObject(row));
    ...
    public void Update(HttpStatusCode statuscode, string message, decimal amount, int qty)
    {
        ...
        Console.WriteLine("[SQLServer-Json] Read a entry. [Entry={0}]", JsonConvert.SerializeObject(row));

        row.response.StatusCode = statuscode;
        row.response.Message = message;
        row.response.Amount = amount;
        row.response.Qty = qty;
        Console.WriteLine("[SQLServer-Json] Modified entry. [Entry={0}]", JsonConvert.SerializeObject(row));

        _ = connection.Update(row);
        var updatedRow = connection.Get<APIResponseHistory>(row.id);
        Console.WriteLine("[SQLServer-Json] Entry updated. [Entry={0}]", JsonConvert.SerializeObject(updatedRow));
    ...
    public void InsertNull()
    {
        ...
        var id = connection.Insert(apiResponseHistory);
        Console.WriteLine("[SQLServer-Json] New entry created with NULL. [Entry={0}]", JsonConvert.SerializeObject(apiResponseHistory));
    ...
    public void ReadAll()
    {
        ...
        Console.WriteLine("[SQLServer-Json] Read all. [Entries={0}]", JsonConvert.SerializeObject(rows));
    ...
    public void ReadAggregatedJsonB()
    {
        ...
        var rows = connection.QueryFirst<List<APIResponse>>("select CONCAT('[',  string_agg(response, ','), ']') as responses from apiresponsehistories");
        Console.WriteLine("[SQLServer-Json] Read aggragated. [Entries={0}]", JsonConvert.SerializeObject(rows));
    ...
    public void ReadList()
    {
        ...
        var rows = connection.Query<APIResponseView>("select CONCAT('[',  string_agg(response, ','), ']') as responses from apiresponsehistories");
        Console.WriteLine("[SQLServer-Json] Read into view. [View={0}]", JsonConvert.SerializeObject(rows));
