...
public class PostgreSQLJsonTests
{
    ...
    public void OneTimeSetup()
    {
        ...
        using (var connection = GetDbConnection())
            connection.Execute(@"create table if not exists apiresponsehistories(id serial primary key, parameters json null, response json null)");

        Console.WriteLine("[PostgreSQL-Json] One Time Setup Completed");
    }

    [SetUp]
    public void Setup()
    {
        Console.WriteLine("[PostgreSQL-Json] Test Setup Completed");
    }

    [TearDown]
    public void TearDown()
    {
        Console.WriteLine("[PostgreSQL-Json] Test Completed");
    }
    ...
    public void OneTimeTearDown()
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Full Test Completed");
    }
    ...
    public void Insert(HttpStatusCode statuscode, string message, decimal amount, int qty, string requestId, string requstPath)
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] New entry created. [Entry={0}]", JsonConvert.SerializeObject(apiResponseHistory));

        var row = connection.Get<APIResponseHistory>(id);
        Console.WriteLine("[PostgreSQL-Json] Read created entry. [Entry={0}]", JsonConvert.SerializeObject(row));
    ...
    public void Update(HttpStatusCode statuscode, string message, decimal amount, int qty)
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Read a entry. [Entry={0}]", JsonConvert.SerializeObject(row));

        row.response.StatusCode = statuscode;
        row.response.Message = message;
        row.response.Amount = amount;
        row.response.Qty = qty;
        Console.WriteLine("[PostgreSQL-Json] Modified entry. [Entry={0}]", JsonConvert.SerializeObject(row));

        _ = connection.Update(row);
        var updatedRow = connection.Get<APIResponseHistory>(row.id);
        Console.WriteLine("[PostgreSQL-Json] Entry updated. [Entry={0}]", JsonConvert.SerializeObject(updatedRow));
    ...
    public void InsertNull()
    {
        ...
        var id = connection.Insert(apiResponseHistory);
        Console.WriteLine("[PostgreSQL-Json] New entry created with NULL. [Entry={0}]", JsonConvert.SerializeObject(apiResponseHistory));
    ...
    public void ReadAll()
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Read all. [Entries={0}]", JsonConvert.SerializeObject(rows));
    ...
    public void ReadOneKey()
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Read only one key. [key Values={0}]", JsonConvert.SerializeObject(rows));
    ...
    public void ReadAggregatedJsonB()
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Read aggragated. [Entries={0}]", JsonConvert.SerializeObject(rows));
    ...
    public void ReadList()
    {
        ...
        Console.WriteLine("[PostgreSQL-Json] Read into view. [View={0}]", JsonConvert.SerializeObject(rows));
